    ...
    mount: function (container, api) {
      ...
      function allGames() {
        var out = games.map(function (g) {
          // A Solo word/sentence game also runs in Live Class Mode when it has a
          // live adapter (Quiz-Blitz and Memory both do), so tag it accordingly.
          return { id: g.id, name: g.name, emoji: g.emoji, color: g.color,
                   kind: g.contentType === "sentences" ? "sentences" : "words",
                   live: !!(window.LiveGames && window.LiveGames[g.id]) };
        });
