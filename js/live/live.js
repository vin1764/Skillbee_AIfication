...
window.LiveMode = (function () {
  ...
  function landing() {
    ...
    show(screen("live-center", [
      ...
      el("div", { class: "live-choice" }, [
        ...
        el("button", { class: "live-choice-card join", on: { click: playerJoin } }, [
          el("div", { class: "live-big-emoji", text: "📱" }),
  ...
  function hostSetup(roster) {
    ...
    var wrap = screen("", [
      el("button", { class: "back-link", html: "← Back", on: { click: hostRosters } }),
      el("h2", { class: "live-title", text: roster.name }),
      el("p", { class: "live-sub", text: "Pick a game and a topic, then start the room." })
    ]);
  ...
  /* ============================================================
     ...
     ============================================================ */
  function hostRun(code, adapter, rounds) {
    stop();
    var sess = null;
    ...
    function renderReveal(i, r, results, scores) {
      var correctList = results.filter(function (x) { return x.correct; });
      show(screen("host", [
        ...
        el("div", { class: "reveal-fast" }, correctList.slice(0, 5).map(function (x, idx) {
          return el("div", { class: "reveal-fast-row" }, [
            el("span", { class: "rf-rank", text: "#" + (idx + 1) }),
            el("span", { class: "rf-name", text: x.name }),
            el("span", { class: "rf-pts", text: "+" + x.points })
          ]);
        })),
        el("button", { class: "btn primary big", text: "Show leaderboard ▶", on: { click: function () { leaderboard(); } } })
      ]));
    }

    function leaderboard() {
      window.LiveDB.updateSession(code, { status: "leaderboard" });
      var scores = sess.scores || {};
      var rows = (sess.students || []).map(function (s) { return { name: s.name, pts: scores[s.id] || 0 }; })
        .sort(function (a, b) { return b.pts - a.pts; });
      var last = sess.questionIndex + 1 >= rounds.length;
      show(screen("host", [
        el("h2", { class: "live-title", text: "Leaderboard" }),
        el("div", { class: "board-list" }, rows.slice(0, 10).map(function (row, i) {
          return el("div", { class: "board-row" + (i < 3 ? " top" : "") }, [
    ...
    function podium() {
      ...
      if (sess.persistMode === "continue") window.LiveDB.saveLeaderboard(sess.rosterId, scores);
      kit.confetti(); kit.beep("win");
      var top3 = rows.slice(0, 3);
      show(screen("host", [
        el("h2", { class: "live-title", text: "🏆 Final results" }),
        ...
        el("button", { class: "btn primary", text: "Done", on: { click: function () { closeRoom(); } } })
      ]));
    }
  ...
  function playerJoin(prefill) {
    ...
    var codeInput = el("input", {
      class: "code-input",
      attrs: { type: "text", maxlength: 4, placeholder: "CODE", autocapitalize: "characters", value: prefill || "" }
    });
